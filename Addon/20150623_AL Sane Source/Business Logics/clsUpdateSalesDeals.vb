...
Public Class clsUpdateSalesDeals
    Inherits clsBase
...
#Region "Methods"
    Private Sub LoadForm()
        oApplication.Utilities.LoadForm(xml_SalesDeals, frm_SalesDeals)
        ...
        oForm.DataSources.UserDataSources.Add("Quarter", SAPbouiCOM.BoDataType.dt_SHORT_TEXT)
        ...
        oCombobox.DataBind.SetBound(True, "", "Quarter")

        oCombobox.ValidValues.Add("", "")
        oCombobox.ValidValues.Add("1", "Fisrt Quarter")
        oCombobox.ValidValues.Add("2", "Second Quarter")
        oCombobox.ValidValues.Add("3", "Third Quarter")
        oCombobox.ValidValues.Add("4", "Fourth Quarter")
        oCombobox.ExpandType = SAPbouiCOM.BoExpandType.et_DescriptionOnly
...
#End Region
    ...
    Private Function AddtoUDT1(ByVal aform As SAPbouiCOM.Form) As Boolean
        Try
            ...
            intQuarter = oCombobox.Selected.Value
            Dim strstardate, strenddate As String
            Select Case intQuarter
                Case "1"
                    strstardate = intYear - 1.ToString("0000") & "-06-01"
                    strenddate = intYear - 1.ToString("0000") & "-11-30"
                Case "2"
                    strstardate = intYear - 1.ToString("0000") & "-09-01"
                    strenddate = intYear.ToString("0000") & "-02-28"
                Case "3"
                    strstardate = intYear - 1.ToString("0000") & "-12-01"
                    strenddate = intYear.ToString("0000") & "-05-30"
                Case "4"
                    strstardate = intYear.ToString("0000") & "-03-01"
                    strenddate = intYear.ToString("0000") & "-08-31"
            End Select

            'Get GeneralService (oCmpSrv is the CompanyService)
            oGeneralService = oCompanyService.GetGeneralService("FINALQUOTA")
            Dim oTestRs1, oSalesData As SAPbobsCOM.Recordset
            oSalesData = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oTestRs1 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            oTestRs1.DoQuery("Select Code,U_TERRITORY from [@QUOTA_MD] order by Code")
            For intLoop As Integer = 0 To oTestRs1.RecordCount - 1
                oApplication.Utilities.Message("Processing Code : " & oTestRs1.Fields.Item(0).Value.ToString.ToUpper, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                ...
                strCode = oTestRs1.Fields.Item("Code").Value
                otestRs.DoQuery("SElect * from [@QUOTA_MD] where Code='" & strCode & "'")
                ...
                Else
                    ...
                    oGeneralData.SetProperty("U_TERRITORY", oTestRs1.Fields.Item("U_TERRITORY").Value)
                    blnRecordExists = False
                End If
                oChildren = oGeneralData.Child("QUOTA_ROWDETAILS")
                ...
                otestRs.DoQuery("Select * from [@QUOTA_RowDetails] where Code='" & strCode & "' and U_Year=" & intYear)
                If otestRs.RecordCount > 0 Then
                ...
                End If
                Dim dblValue, dblMin, dblMax As Double
                Dim strString As String
                oSalesData.DoQuery("select SUM(isnull(U_SALES_D,0) ) from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "'")
                dblValue = oSalesData.Fields.Item(0).Value
                ' strString = "select Min(isnull(U_SALES_D,0 )) from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "'"
                strString = " select Min(x.Total)  from  (select sum((isnull(U_SALES_D,0 ))) 'Total',MONTH(U_Date) 'Month' from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "' group by MONTH(U_Date) ) x"
                oSalesData.DoQuery(strString)
                dblMin = oSalesData.Fields.Item(0).Value
                ' oSalesData.DoQuery("select Max(isnull(U_SALES_D,0 )) from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "'")
                strString = " select max(x.Total)  from  (select sum((isnull(U_SALES_D,0 ))) 'Total',MONTH(U_Date) 'Month' from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "' group by MONTH(U_Date) ) x"
                oSalesData.DoQuery(strString)
                dblMax = oSalesData.Fields.Item(0).Value
                dblValue = dblValue - dblMin
                dblValue = dblValue - dblMax
                'oSalesData.DoQuery("select SUM(isnull(U_SALES_D,0 ) from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "'")
                Dim dblNumberofMonths As Double
                strString = " select *  from  (select sum((isnull(U_SALES_D,29999 ))) 'Total',MONTH(U_Date) 'Month' from [@DAILY_FACTS_DETAILS] where Code='" & strCode & "' and U_Date between '" & strstardate & "' and '" & strenddate & "' group by MONTH(U_Date) ) x"
                oSalesData.DoQuery(strString)
                dblNumberofMonths = 0
                If oSalesData.RecordCount > 0 Then
                    dblNumberofMonths = oSalesData.RecordCount
                End If
                Select Case dblNumberofMonths
                    Case 6
                        dblNumberofMonths = 4
                        dblValue = dblValue / dblNumberofMonths
                    Case 5
                        dblNumberofMonths = 3
                        dblValue = dblValue / dblNumberofMonths
                    Case 4
                ...
                'oChild.SetProperty("Code", strCode)
                oChild.SetProperty("U_YEAR", intYear.ToString)
                Select Case intQuarter
                    Case "1"
                        oChild.SetProperty("U_FIRSTQUARTER", CInt(dblValue))
                    Case "2"
                        oChild.SetProperty("U_SECONDQUARTER", CInt(dblValue))
                    Case "3"
                        oChild.SetProperty("U_THIRDQUARTER", CInt(dblValue))
                    Case "4"
                        oChild.SetProperty("U_FOURTHQUARTER", CInt(dblValue))
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_SalesDeals Then
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "3" Then
                                    If oApplication.SBO_Application.MessageBox("Do you want to Calculate Sales Deals  ?.", , "Yes", "No") = 2 Then
                                        BubbleEvent = False
                                        Exit Sub
                                    Else
